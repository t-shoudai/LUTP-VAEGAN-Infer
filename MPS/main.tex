...
無順序ラベル付き木を対象とするパターンマイニングは，XML 文書処理や生命情報科学などを背景として広く研究されてきた．Zaki~\cite{zaki2005} は頻出埋め込み無順序部分木のマイニングアルゴリズム SLEUTH を，Chehreghani ら~\cite{chehreghani2007} は最大埋め込み無順序木のトップダウン探索手法 TDU を提案している．これらは大規模木データベースから頻出部分構造を列挙することを主目的としており，既知パターンの言語を学習・生成する本研究とは問題設定が異なる．石灘ら~\cite{ishinada2026} は，学習済み GCN を不完全な教師として用い，その所属性質問への応答に基づいて線形無順序木パターンを同定する質問学習モデルを提案している．本研究における下流推論アルゴリズムは，この枠組みで確立されたパターン照合・精密化の考え方を基盤としている．

グラフ生成への深層学習の応用としては，GraphVAE~\cite{simonovsky2018} が隣接行列を変分オートエンコーダによりモデル化し，JT-VAE~\cite{jin2018} が分子グラフを Junction Tree を介して生成する手法を示している．TD-GEN~\cite{shirzad2022} は木分解に基づくグラフ生成を提案する．これらは一般グラフや分子グラフの生成を主対象としており，本研究のようにパターン言語の正例分布を学習対象とする設定や，seed に対する学習安定性を中心的に扱う観点は異なる．

グラフ上の表現学習としては，Kipf ら~\cite{kipf2017} の GCN や Hamilton ら~\cite{hamilton2017} の GraphSAGE が基本的な枠組みとして広く用いられている．本研究では，無順序木のラベル付きエッジを多関係構造として明示的に扱うため，Schlichtkrull ら~\cite{schlichtkrull2018} による関係型グラフ畳み込み（RGCNConv）をエンコーダに採用する．

データ拡張による表現安定化については，画像認識~\cite{shorten2019} や自然言語処理~\cite{wei2019} において有効性が示されている．無順序木では兄弟ノードの順序は本来意味を持たないが，BFS 再番号付けにより等価な木が異なる入力表現として与えられうる．この種の表現変動を主題化し，無順序木パターン生成の安定性を系統的に検討した研究は，著者らの知る限り限られている．本研究では，この問題をデータ拡張により緩和することを提案する．
...
\begin{figure*}[t]
\begin{center}
\includegraphics[width=0.85\linewidth]{vae-gan-architecture-clean.jpeg}
\end{center}
...
主要な実験設定を表~\ref{tab:config} に示す．
提案モデルは，Encoder，Decoder，Discriminator からなる VAE-GAN 型構成を採用する．
Encoder には RGCNConv，Decoder には TAGConv，Discriminator にはGraphConv を用いた．学習時の主な設定は表~\ref{tab:config} の通りであり，KL 項については $\beta$-VAE 的なアニーリングを導入し，学習初期から一様に強い正則化を課すのではなく，段階的に重みを増加させることで
学習の安定化を図った．学習エポック数は最大 500 とし，検証損失が 30 エポック改善しない場合に早期終了（early stopping）を適用した．
...
\centering
\small
\caption{各設定における encoded\_z および empirical latent の結果}
\label{tab:results}
\begin{tabular}{lrr|rrr|rrr}
\toprule
& & & \multicolumn{3}{c|}{encoded\_z} & \multicolumn{3}{c}{empirical latent} \\
設定 & 正例数 & 拡張 & 最小 & 最大 & 平均 & 最小 & 最大 & 平均 \\
\midrule
np500\_bank100   & 500  & 1 & 0.910 & 1.000 & 0.983 & 0.930 & 1.000 & 0.988 \\
np1000\_bank100  & 1000 & 1 & 0.975 & 1.000 & 0.997 & 0.980 & 1.000 & 0.998 \\
np2000\_bank100  & 2000 & 1 & 0.963 & 1.000 & 0.995 & 0.990 & 1.000 & 0.999 \\
np1000\_bankfull & 1000 & 1 & 0.940 & 1.000 & 0.988 & 0.920 & 1.000 & 0.985 \\
np1000\_bank20   & 1000 & 1 & 0.985 & 1.000 & 0.996 & 0.980 & 1.000 & 0.998 \\
np1000\_aug0\_bank100 & 1000 & 0 & 0.925 & 1.000 & 0.986 & 0.890 & 1.000 & 0.981 \\
np1000\_aug1\_bank100 & 1000 & 1 & 0.955 & 1.000 & 0.990 & 0.920 & 1.000 & 0.985 \\
\bottomrule
\end{tabular}
\end{table*}
...
表\ref{tab:results}に，各設定・各 seed における encoded\_z および empirical\_latent の値を示す．
...
\bibitem{ishinada2026}
石灘 洸樹, 正代 隆義, 内田 智之, 松本 哲志,
``木構造パターン発見のためのグラフ畳み込みネットワークを教師とする質問学習モデル,''
情報処理学会論文誌, Vol.~66, to appear (2026).
